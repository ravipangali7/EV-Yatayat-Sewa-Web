...
export default function UserShortTripDetail() {
  ...
  const hasTrip = !!booking?.trip && !!booking?.trip_details;
  const tripEnded = hasTrip && !!booking?.trip_details?.end_time;
  const canLiveTrack = hasTrip && !tripEnded;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="px-5 pt-6 pb-24 space-y-4">
        ...
        {booking.destination_place_details && (
          <div className="bg-white dark:bg-card/80 rounded-2xl border border-border/50 p-4 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Destination</p>
            <DetailRow label="Place" value={booking.destination_place_details.name ?? "—"} />
          </div>
        )}
        ...
        {(booking.trip_distance != null || booking.trip_duration != null || booking.trip_amount != null) && (
          <div className="bg-white dark:bg-card/80 rounded-2xl border border-border/50 p-4 shadow-sm">
            ...
            {booking.trip_distance != null && <DetailRow label="Distance" value={`${Number(booking.trip_distance).toFixed(2)} km`} />}
            ...
            {booking.trip_amount != null && <DetailRow label="Amount" value={`Rs. ${Number(booking.trip_amount).toLocaleString()}`} accent="green" />}
          </div>
        )}

        <div className="bg-white dark:bg-card/80 rounded-2xl border border-border/50 p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">Live tracking</p>
          {!hasTrip && <p className="text-sm text-muted-foreground">Tracking not available. No trip linked to this booking.</p>}
